...
export const request = async (
  method: string,
  path: string,
  { body, userId, accessToken }: any
): Promise<Response> => {
  const version = process.env.FAB_VERSION as string;
  const userAgent = (process.env.FAB_USER_AGENT as string).replace(
    "%version%",
    version
  );
  ...
  try {
    ...
    if (data.error) {
      ...
      if (data.error.error_code === 1002) {
        console.info(
          chalk.red.bold(
            "Fab version mismatch, restarting to trigger a version fetch."
          )
        );
        process.exit();
      }
...
export const fetchFabVersion = async (): Promise<string> => {
  ...
  try {
    ...
    console.info(
      chalk.red("Error fetching Fab version from Play Store:", chalk.bold(e))
    );
  }
