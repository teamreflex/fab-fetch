...
const parseMessage = (message: FabMessage): ParsedMessage => {
  ...
  const parsed = {
    ...
    updatedAt: !!message.postcard
      ? message.postcard.updatedAt
      : message.letter.updatedAt,
...
const fetchMessagesByArtist = async (artist: Artist): Promise<FabMessage[]> => {
  ...
  const { data, error } = await request(
    "get",
    `artists/${artist.artistId}/messages`,
    {
      userid: userId,
...
const fetchMessages = async (): Promise<FabMessage[]> => {
  ...
  const messages = await Promise.all(
    artists.map((artist) => fetchMessagesByArtist(artist))
  );
...
const payForMessage = async (message: FabMessage): Promise<ParsedMessage> => {
  ...
  const { data, error } = await request(
    "get",
    `users/${userId}/message/${message.id}`,
    {
      userid: userId,
      accessToken: accessToken,
    }
...
const buildMessage = async (parsedMessage: ParsedMessage): Promise<Message> => {
  ...
  message.type = parsedMessage.isPostcard
    ? MessageType.POSTCARD
    : MessageType.LETTER;
  message.artist = await AppDataSource.getRepository(Artist).findOne({
    where: { artistId: parsedMessage.user.id },
  });
...
export const saveMessages = async (): Promise<Message[]> => {
  ...
  // fetch messages
  const unfilteredMessages = (await fetchMessages()).filter(
    (m) => m.user.artist.isTerminated === "N"
  );

  // we want to operate on already saved messages, but filter on new comments
  const messagesWithNewComments = unfilteredMessages.filter(
    (m) => m.isNewArtistUserComment === "Y"
  );
  ...
  for (let i = 0; i < unfilteredMessages.length; i += chunkSize) {
    ...
    filteredMessages.push(
      ...chunk.filter(
        (message) =>
          inDatabase.find((m) => m.messageId === message.id) === undefined
      )
    );
  }
  ...
  for (const fabMessage of filteredMessages) {
    ...
    if (result !== DownloadResult.CONNECTION_ERROR) {
      ...
      console.info(
        chalk.green(
          "Saved message from:",
          chalk.bold.cyan(parsed.user.enName),
          `(Found ${parsed.media.length} images/videos)`
        )
      );
    }
  }

  // scan comment threads for content
