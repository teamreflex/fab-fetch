...
const fetchFromRemote = async (): Promise<Artist[]> => {
  ...
  for (const remoteArtist of followedArtists) {
    ...
    if (localArtist === null) {
      ...
      if (
        localArtist.isTerminated !==
        (remoteArtist.artist.isTerminated === "Y")
      ) {
...
const handleProfilePicture = async (
  ...
  const profilePicture = await AppDataSource.getRepository(
    ProfilePicture
  ).findOne({
    where: {
...
const handleProfileBanner = async (
  user: FabUser,
  artist: Artist,
  twitter?: TwitterApi
): Promise<void> => {
  ...
  const profileBanner = await AppDataSource.getRepository(
    ProfileBanner
  ).findOne({
    where: {
