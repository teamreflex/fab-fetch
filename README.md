...
### `IMAGE_COMMENTS_ENABLED` (boolean)

Saves image comment files or not. These get placed in the `image_comments/` folder, and is affected by the `MONTHLY_FOLDERS` option.
...
console.log(
  `https://dnkvjm1f8biz3.cloudfront.net/images/letter/${letter}/${time.toFormat(
    "X"
  )}_${time.toFormat("yyyyMMddHHmmss")}_1_f.jpg`
);
```
